...
class HuggingFaceDatasetSmokeTest(unittest.TestCase):
    def test_fit_dataset_resolves_and_loads_real_hf_repo(self):
        ...
        with patch("unified_stpp.runner.STPPRunner.fit", return_value=result) as fit_mock:
            fit.execute(args)
        ...
        self.assertEqual(fit_mock.call_args.kwargs["dataset_id"], "bold5000-stpp")
    ...
    def test_tune_dataset_resolves_and_loads_real_hf_repo(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            def _fake_run_hpo(*, config_dict, tuning, train_path, val_path, dataset_id, return_analysis):
                ...
                self.assertEqual(dataset_id, "bold5000-stpp")
                return STPPConfig(**config_dict), None

            with patch("unified_stpp.benchmark.hpo.run_hpo", side_effect=_fake_run_hpo):
                tune.execute(args)
    ...
    def test_bench_dataset_resolves_and_loads_real_hf_repo(self):
        ...
        def _fake_benchmark(*args, **kwargs):
            ...
            self.assertEqual(sorted(splits.keys()), ["bold5000-stpp"])
            self.assertGreater(len(splits["bold5000-stpp"][0]), 0)
            self.assertGreater(len(splits["bold5000-stpp"][1]), 0)
            return bench_instance

        with patch("unified_stpp.benchmark.Benchmark", side_effect=_fake_benchmark):
            bench.execute(args)
