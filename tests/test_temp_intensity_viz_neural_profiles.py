...
from unified_stpp.evaluation.surface.diagnostics import (
    ...
    DEFAULT_X_NSTEP,
    DEFAULT_Y_NSTEP,
    build_future_query_grid,
    history_overlay_z_level,
    resolve_neural_exact_profile,
)
...
class TestNeuralSTPPVizProfiles(unittest.TestCase):
    ...
    def test_future_query_grid_excludes_last_history_time(self):
        grid = build_future_query_grid(last_t=10.0, horizon=2.0, n_steps=4)
    ...
    def test_history_overlay_stays_on_zero_surface(self):
        ...
        self.assertEqual(history_overlay_z_level(frame), 0.0)

    def test_history_overlay_uses_actual_surface_range(self):
        frame = [[2.0, 4.0], [6.0, 10.0]]
        self.assertAlmostEqual(history_overlay_z_level(frame), 2.0 + 0.88 * (10.0 - 2.0), places=6)
