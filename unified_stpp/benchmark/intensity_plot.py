...
plot_bench_intensities()
    ...
    MP4) of λ*(t, s) evolving over time.
...
def _model_lam(
    ...
    *,
    ...
    """Evaluate model λ*(t_query, ·) on a meshgrid. Returns (n_grid, n_grid)."""
...
def plot_bench_intensities(
    ...
    for ds_id in dataset_ids:
        ...
        xb = (float(xq[0] - .1*xs), float(xq[1] + .1*xs))
        yb = (float(yq[0] - .1*ys), float(yq[1] + .1*ys))
        ...
        def _update(frame_idx: int):
            for col, ax in enumerate(axes):
                ...
                ax.plot_surface(xx, yy, grids[frame_idx][col],
                                ...
                                linewidth=0, antialiased=False)
            title_obj.set_text(
                f"λ*(t, s | H)  —  dataset: {ds_id}  |  t = {t_snaps[frame_idx]:.2f}"
            )
            return []
