"""Thin state model for upstream-faithful AutoSTPP preprocessing."""
...
@register_state("auto_stpp_faithful")
class AutoSTPPFaithfulStateModel(StateModel):
    ...
    def __init__(
        ...
        **_,
        ...
        if int(spatial_dim) != 2:
            raise ValueError(
                f"AutoSTPP faithful state model requires spatial_dim=2, got {spatial_dim}."
            )
    ...
    def append_sampling_event(
        ...
        *,
        ...
        if times_raw.shape[0] != 1:
            raise NotImplementedError(
                "AutoSTPP faithful sampling-state append currently supports batch size 1 only."
            )
