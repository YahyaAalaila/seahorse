"""
AutoSTPP Decoder — intensity-based spatiotemporal point process decoder.
...
class AutoIntDecoder(Decoder):
    ...
    def _compensator(self, z: Tensor, tau: Tensor) -> Tensor:
        """
        Λ(t_prev, t) = μ·|S|·τ + [box_integral(τ) − box_integral(0)].
        ...
        base_comp = self._mu() * area * tau.squeeze(-1)   # (B,)
    ...
    def log_prob(
        ...
        return torch.log(self._mu() + f + 1e-8)              # (B,)
    ...
    def nll(
        ...
        tau     = (t - t_prev).clamp(min=1e-6)             # (B, 1)
        comp    = self._compensator(z, tau)                 # (B,)
        return -log_lam + comp
