"""Construction config for the canonical AutoSTPP preset."""
...
def _compute_paper_stats_from_dataset(dataset: Any, spatial_dim: int) -> dict[str, Any]:
    ...
    sequences = getattr(dataset, "stat_sequences", getattr(dataset, "sequences", []))
...
class AutoSTPPConfig(BaseModelConfig):
    ...
    def from_dict(
        ...
        *,
        ...
        if spatial_dim != 2:
            raise ValueError(
                f"auto_stpp requires spatial_dim=2, got {spatial_dim}."
            )
        ...
        if dec_type != "autoint":
            raise ValueError(
                "AutoSTPP build expects decoder.type='autoint', "
                f"got '{dec_type}'."
            )
