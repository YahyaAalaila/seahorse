"""
NeuralTPP Backbone — faithful NeuralPointProcess temporal model.
...
Self-contained temporal point process owning:
  ...
  - intensity: sigmoid(mlp(h) − 2.0) × 50   (IntensityODEFunc faithful; 2-layer MLP)
  - joint ODE: d/dt [h, Λ] = [f(h), λ(h(t))]
  - main API: sequence_nll_and_states(events, lengths) → (B,L), (B,L,h)

Used by neural_stpp_jump_sc and neural_stpp_attn_sc presets.
Bypasses the standard Encoder + NeuralODEDynamics + Updater +
CumulativeHazardTemporal path entirely.
...
from .dynamics.neural_ode import euler_solve

class NeuralTPPODEFunc(nn.Module):
    """
...
class _AugRHS(nn.Module):
    ...
    State layout: [h (hidden_dim) | Λ (1) | E (1)]
        ...
        E: kinetic energy accumulator ∫ (‖dh‖² + λ²)/(h+1) dt
           ...
           When energy_regularization=0 on the backbone, E is tracked but the
    ...
    def __init__(self, ode_func: NeuralTPPODEFunc, intensity_net: nn.Module):
        super().__init__()
...
class HiddenStateUpdate(nn.Module):
    """
    Configurable jump update for NeuralTPP.
...
class NeuralTPPBackbone(nn.Module):
    """
    Self-contained NeuralPointProcess temporal backbone.

    Owns the complete temporal generative model:
        h(0)       = _init_state                      (learnable, no encoder)
        dh/dt      = tanh(net(h))                     (time-independent ODE)
        λ(t)       = sigmoid(mlp(h(t)) − 2.0)×50    (IntensityODEFunc; 2-layer MLP)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ode_func = NeuralTPPODEFunc(
            hidden_dim,
        ...
        self._debug_neural_tpp = os.getenv("UNIFIED_STPP_DEBUG_NSTPP", "0") == "1"
        ...
        self._debug_neural_tpp_max_steps = max(
            1, int(os.getenv("UNIFIED_STPP_DEBUG_NSTPP_MAX_STEPS", "3"))
        )
    ...
    def sequence_nll_and_states(
        ...
        """
        Full NeuralPointProcess forward pass over a batch of sequences.
        ...
        return temporal_nll, h_seq_pre, energy_reg
