"""Event model for the upstream-faithful AutoSTPP preset."""
...
from .auto_stpp_faithful_kernel import AutoSTPPFaithfulCuboid


@register_event("auto_stpp_faithful")
class AutoSTPPFaithfulEventModel(EventModel):
    ...
    def __init__(
        ...
        **_,
        ...
        if int(spatial_dim) != 2:
            raise ValueError(
                f"AutoSTPP faithful event model requires spatial_dim=2, got {spatial_dim}."
            )
        self.kernel = AutoSTPPFaithfulCuboid(
    ...
    def _get_state_term(state_ctx: StateContext, key: str) -> Tensor:
        ...
        if val is None:
            raise ValueError(f"AutoSTPPFaithfulEventModel requires state['{key}'].")
        if not isinstance(val, Tensor):
            raise TypeError(
                f"AutoSTPPFaithfulEventModel expects tensor for state['{key}']."
            )
        return val
    ...
    def _broadcast_or_match(tensor: Tensor, batch_size: int, name: str) -> Tensor:
        ...
        raise ValueError(
            f"AutoSTPPFaithfulEventModel state['{name}'] has batch={tensor.shape[0]} "
    ...
    def _compute(
        ...
        *,
        ...
        if self.lookahead != 1:
            raise ValueError(
                f"AutoSTPPFaithfulEventModel currently supports lookahead=1, got {self.lookahead}."
            )
        ...
        if not torch.all(lambs_sum > 0):
            raise ValueError("AutoSTPP faithful intensity became non-positive.")
    ...
    def intensity(
        ...
        *,
        ...
        if query_locations.ndim != 2:
            raise ValueError(
                "AutoSTPPFaithfulEventModel.intensity expects query_locations with shape (B, d)."
            )
        if device is None:
