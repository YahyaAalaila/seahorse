...
from unified_stpp.data.contract import validate_sequence_records
from unified_stpp.utils import load_jsonl


_REPO_ROOT = Path(__file__).resolve().parents[2]
_DEFAULT_HF_DATASET_REPO = os.environ.get("UNIFIED_STPP_HF_DATASET_REPO")
...
def _build_catalog() -> dict[str, CuratedDatasetSpec]:
    ...
    specs.extend(
        _leaf_specs(
            ...
            "unified_stpp_hawkesnest_v20260409",
            tuple(
    ...
    specs.extend(
        _leaf_specs(
            ...
            "unified_stpp_hawkesnest_families_v1",
            tuple(
    ...
    specs.extend(
        _leaf_specs(
            "hawkesnest_hard_v2",
            "unified_stpp_hawkesnest_hard_v2",
            tuple(
                [f"pulse_P{i}" for i in range(6)]
                + [f"echo_E{i}" for i in range(6)]
                + [f"regime_R{i}" for i in range(6)]
