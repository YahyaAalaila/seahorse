...
def get_state_cls(key: str) -> type:
    ...
    Imports ``seahorse.models.state_models`` on the first call if the key
    is not yet present, which triggers all ``@register_state`` decorators.
    """
    if key not in _STATE_REGISTRY:
        import seahorse.models.state_models  # noqa: F401
    if key not in _STATE_REGISTRY:
        raise ValueError(
...
def get_event_cls(key: str) -> type:
    ...
    if key not in _EVENT_REGISTRY:
        import seahorse.models.event_models  # noqa: F401
    if key not in _EVENT_REGISTRY:
        raise ValueError(
...
def get_spatial_cls(key: str) -> type:
    ...
    if key not in _SPATIAL_REGISTRY:
        import seahorse.models.spatial_models  # noqa: F401
    if key not in _SPATIAL_REGISTRY:
        raise ValueError(
