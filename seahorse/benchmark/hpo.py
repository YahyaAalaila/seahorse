...
:meth:`seahorse.config.tuning.TuningConfig.parse_config`.
"""
...
def _mem_debug_enabled() -> bool:
    return os.environ.get("SEAHORSE_HPO_MEM_DEBUG", "").lower() not in {
        "",
        "0",
        "false",
        "no",
...
def build_hpo_manifest(
    *,
    ...
    manifest: dict[str, Any] = {
        ...
        "container_image_tag": os.environ.get("SEAHORSE_IMAGE_TAG"),
        "best_config_path": (
...
def run_hpo(
    ...
    *config_dict* unchanged.
    ...
    try:
        ...
        raise ImportError(
            "HPO requires Ray Tune: pip install 'seahorse-stpp[hpo]'"
        ) from exc
